...
@Component({
  selector: 'app-anonymize-vault-transaction',
  templateUrl: './anonymize-vault-transaction.component.html',
  styleUrls: ['./anonymize-vault-transaction.component.scss']
})
export class AnonymizeVaultTransactionComponent implements OnInit, OnDestroy {
  @Input() display: any;
  ...
  private log: any = Log.create('anonymize-vault-transaction.component');
          ...
          this._rpc.call(ApiEndpoints.ListTransactions, ['*', 100])
            .subscribe(recentTransInfo => {
              let res = recentTransInfo.filter(item => {
                if (item.is_anonymized) return true;
              });
